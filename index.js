...
bot.onText(/\/casualties/, (msg) => {
  const chatId = msg.chat.id;
  const history = messageHistory[chatId];
  const prompt = `Роль: Ти — аналітик текстових зведень про обстріли. Твоє завдання — вирахувати точну загальну кількість загиблих та поранених цивільних і видати результат суворо за визначеним шаблоном.

Правило аналізу даних та повторів:
1. Уважно аналізуй кожне повідомлення: Окремі новини можуть містити оперативні дані (наприклад, по місту), а інші — зведення за добу (по області).
2. Рахуй усі унікальні випадки: Якщо цифри в різних повідомленнях відображають різні події або різні проміжки часу (наприклад, свіжий прильот прямо зараз і підсумок за минулу добу), вони обов'язково мають додаватися. Враховуй усі деталі, щоб не загубити жодного постраждалого.
3. Об'єднуй географію: Збери назви всіх областей, згаданих у тексті.

Формат виводу (Видавай ТІЛЬКИ цей рядок, без зайвих привітань чи пояснень):
⚔️ Втрати серед цивільних: Загалом загинуло [Сума] осіб та [Сума] осіб отримали поранення внаслідок ворожих атак у [Перелік областей через кому у родовому відмінку] областях.
